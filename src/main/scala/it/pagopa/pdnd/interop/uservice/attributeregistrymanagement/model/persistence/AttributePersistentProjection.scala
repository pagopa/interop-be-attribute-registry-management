...
class AttributePersistentProjection(system: ActorSystem[_], entity: Entity[Command, ShardingEnvelope[Command]]) {
  ...
  def projection(tag: String): AtLeastOnceFlowProjection[Offset, EventEnvelope[Event]] = {
    CassandraProjection
      .atLeastOnceFlow(projectionId = ProjectionId("attribute-projections", tag), sourceProvider(tag), handler = flow)
  ...
  val projections: Seq[AtLeastOnceFlowProjection[Offset, EventEnvelope[Event]]] =
    (0 until settings.numberOfShards).map(i =>
      projection(s"pdnd-interop-uservice-attribute-registry-management-persistence-attribute|$i")
    )
