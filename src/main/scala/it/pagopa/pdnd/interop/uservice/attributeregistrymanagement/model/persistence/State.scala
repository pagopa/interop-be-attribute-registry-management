...
import it.pagopa.pdnd.interop.uservice.attributeregistrymanagement.model.persistence.attribute.Certified
...
final case class State(attributes: Map[String, PersistentAttribute]) extends Persistable {
  ...
  def update(updatedAttribute: PersistentAttribute): State = attributes
