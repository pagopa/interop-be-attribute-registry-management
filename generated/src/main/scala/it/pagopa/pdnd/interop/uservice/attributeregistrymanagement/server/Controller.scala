package it.pagopa.pdnd.interop.uservice.attributeregistrymanagement.server

import akka.http.scaladsl.Http
import akka.http.scaladsl.server.Route
import it.pagopa.pdnd.interop.uservice.attributeregistrymanagement.api.AttributeApi
import it.pagopa.pdnd.interop.uservice.attributeregistrymanagement.api.HealthApi
...
class Controller(attribute: AttributeApi, health: HealthApi, validationExceptionToRoute: Option[ValidationException => Route] = None)(implicit system: ActorSystem) {
  ...
  */
  def getBuildInfo: Route =
    path("pdnd-interop-uservice-attribute-registry-management" / "build-info") {
      get {
        complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
      }
   }

  lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-attribute-registry-management" / interfaceVersion) {
    toStrictEntity(strictnessTimeout) {
      extractRequest {
        request =>
          validationFunction(request){
            attribute.route  ~ health.route 
        }
